  ...
  function featchPackage() {
    axios.get('https://entmcq.vertextechnosys.com/api/package/' + id.id)
      .then((res) => {
        ...
        setPrice(data.pricing);
        setDiscount(data.discount);
        setPackage(JSON.parse(data.service_include));
        // setServices(data.userinfo.response);
                          ...
                          <FormGroup>

                            {
                              service.map((obj) => {
                                ...
                                // })
                                if (packages.includes(obj.id)) {
                                  return (
                                    <FormControlLabel control={<Checkbox />} label={obj.service_name} value={obj.id} name="services" id="flexCheckDefault" onChange={(e)=>{handleChange()}} defaultChecked={true}/>
                                  )
                                }
                                else{
                                  return (
